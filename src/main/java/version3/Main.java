...
public class Main {
    ...
    private static Map<String, Stats> parse(ByteBuffer buf, boolean skipFirstLine) {
        ...
        Map<String, Stats> map = new HashMap<>(16_384);
        ...
        while (buf.hasRemaining()) {
            ...
            if (b == ';') {
                ...
                double value =
                        sign * (intPart + fracPart / (double) fracDiv);

                String station =
                        new String(stationBuf, 0, stationLen);

                map.computeIfAbsent(station, k -> new Stats())
                        .add(value);
